...
「目次」の言葉は二つはいらない。後から生成される`summary`のほうだけ残して、`remark-toc`のために入れてある`### 目次`の部分を見えなくしたいのだが、このHTML要素には`id`が付いていない。目次が一番前になることが確約されているなら、記事の中のhタグから`:first-child`で`display: none`にすればいいが、自分は`remark-attributes`を入れているのでこのようにも書ける。
```md
### 目次{: .toc-heading}
```
