...
function n(e, ...t) {
  return e.replace(/\$(\d+)/g, (e, o) => {
    var s;
    const i = parseInt(o, 10) - 1;
    return null !== (s = t[i]) && void 0 !== s ? s : "";
  });
}
class l {
  constructor(e = "info") {
...
class c {
  constructor(e, t = []) {
    ...
    (isNaN(d) || isNaN(r)) && (r = 0),
      isNaN(d) && (d = null !== (o = e.modelId) && void 0 !== o ? o : 0),
      (this.useCDN = c),
      (this.cdnPath = i || ""),
      (this.cubism2Path = n || ""),
      (this.cubism5Path = l || ""),
  ...
  static async initCheck(e, t = []) {
    const o = new c(e, t);
    if (o.useCDN) {
      const e = await fetch(`${o.cdnPath}model_list.json`);
      (o.modelList = await e.json()),
        o.modelId >= o.modelList.models.length && (o.modelId = 0);
      const t = o.modelList.models[o.modelId];
      if (Array.isArray(t))
        o.modelTexturesId >= t.length && (o.modelTexturesId = 0);
      else {
        const e = `${o.cdnPath}model/${t}/index.json`,
          s = await o.fetchWithCache(e);
        if (2 === o.checkModelVersion(s)) {
          const e = await o.loadTextureCache(t);
          o.modelTexturesId >= e.length && (o.modelTexturesId = 0);
        }
      }
    } else
      o.modelId >= o.models.length && (o.modelId = 0),
        o.modelTexturesId >= o.models[o.modelId].paths.length &&
          (o.modelTexturesId = 0);
    return o;
  }
  set modelId(e) {
  ...
  async loadLive2D(e, t) {
    if (this.loading) a.warn("Still loading. Abort.");
      ...
      this.loading = !0;
      try {
        const s = this.checkModelVersion(t);
        if (2 === s) {
        ...
        a.info(`Model ${e} (Cubism version ${s}) loaded`),
          (this.currentModelVersion = s);
      } catch (e) {
  ...
  async loadModel(e) {
    ...
    if (this.useCDN) {
      ...
      Array.isArray(e) && (e = e[this.modelTexturesId]),
        ...
        (o = await this.fetchWithCache(t));
      if (2 === this.checkModelVersion(o)) {
        const t = await this.loadTextureCache(e);
        if (t.length > 0) {
          let e = t[this.modelTexturesId];
          "string" == typeof e && (e = [e]), (o.textures = e);
        }
      }
    } else
      (t = this.models[this.modelId].paths[this.modelTexturesId]),
        (o = await this.fetchWithCache(t));
    await this.loadLive2D(t, o), i(e, 4e3, 10);
  }
  async loadRandTexture(e = "", o = "") {
    const { modelId: s } = this;
    let n = !1;
    if (this.useCDN) {
      const e = this.modelList.models[s];
      if (Array.isArray(e))
        this.modelTexturesId = t(e.length, this.modelTexturesId);
      else {
        const o = `${this.cdnPath}model/${e}/index.json`,
          s = await this.fetchWithCache(o);
        if (2 === this.checkModelVersion(s)) {
          const o = await this.loadTextureCache(e);
          o.length <= 1
            ? (n = !0)
            : (this.modelTexturesId = t(o.length, this.modelTexturesId));
        } else n = !0;
      }
    } else
      1 === this.models[s].paths.length
        ? (n = !0)
        : (this.modelTexturesId = t(
            this.models[s].paths.length,
            this.modelTexturesId,
          ));
    n ? i(o, 4e3, 10) : await this.loadModel(e);
  }
  async loadNextModel() {
...
class d {
      ...
      (this.tools = {
        hitokoto: {
          ...
          callback: async () => {
            const e = await fetch("https://v1.hitokoto.cn"),
              t = await e.json(),
              s = n(o.message.hitokoto, t.from, t.creator);
            i(t.hitokoto, 6e3, 9),
              setTimeout(() => {
                i(s, 4e3, 9);
              }, 6e3);
        ...
        "switch-texture": {
          ...
          callback: () => {
            let t = "",
              s = "";
            o && ((t = o.message.changeSuccess), (s = o.message.changeFail)),
              e.loadRandTexture(t, s);
          },
        },
        photo: {
        ...
        quit: {
          ...
          callback: () => {
            var e;
            const t =
              null === (e = this.config.showToggleAfterQuit) ||
              void 0 === e ||
              e;
            t
            ...
            i(o.message.goodbye, 2e3, 11);
            const s = document.getElementById("waifu");
            s &&
              (s.classList.remove("waifu-active"),
              setTimeout(() => {
                var e;
                if ((s.classList.add("waifu-hidden"), t)) {
                  const e = document.getElementById("waifu-toggle");
                  null == e || e.classList.add("waifu-toggle-active");
                } else
                  null === (e = document.getElementById("waifu-toggle")) ||
                    void 0 === e ||
                    e.remove();
              }, 3e3));
          },
  ...
  registerTools() {
    ...
    for (const t of this.config.tools)
      if (this.tools[t]) {
        const { icon: o, callback: s } = this.tools[t],
...
async function r(t) {
  ...
  let s,
    l = [];
  if (t.waifuPath) {
    const o = await fetch(t.waifuPath);
    (s = await o.json()),
      (l = s.models),
      (function (t) {
        let o,
        ...
        t.seasons.forEach(({ date: t, text: o }) => {
          const s = new Date(),
            i = t.split("-")[0],
            l = t.split("-")[1] || i;
          Number(i.split("/")[0]) <= s.getMonth() + 1 &&
            s.getMonth() + 1 <= Number(l.split("/")[0]) &&
            Number(i.split("/")[1]) <= s.getDate() &&
            s.getDate() <= Number(l.split("/")[1]) &&
            ((o = (o = e(o)).replace("{year}", String(s.getFullYear()))),
            n.push(o));
        }),
          ...
          window.addEventListener("mouseover", (o) => {
            ...
            for (let { selector: n, text: a } of t.mouseover)
              if (
                null === (s = o.target) || void 0 === s ? void 0 : s.closest(n)
              ) {
                if (l === n) return;
                return (
                  (l = n),
                  (a = e(a)),
                  (a = a.replace("{text}", o.target.innerText)),
                  void i(a, 4e3, 8)
                );
              }
          }),
          window.addEventListener("click", (o) => {
            ...
            for (let { selector: n, text: l } of t.click)
              if (
                ...
                return (
                  (l = e(l)),
                  (l = l.replace("{text}", o.target.innerText)),
                  void i(l, 4e3, 8)
                );
          }),
          window.addEventListener("live2d:hoverbody", () => {
            if (l === "live2d:hoverbody") return;
            (l = "live2d:hoverbody"), i(e(t.message.hoverBody), 4e3, 8);
          }),
      ...
      i(
        (function (e, t, o) {
          if ("/" === location.pathname)
            for (const { hour: t, text: o } of e) {
              const e = new Date(),
                s = t.split("-")[0],
                i = t.split("-")[1] || s;
              if (Number(s) <= e.getHours() && e.getHours() <= Number(i))
                return o;
            }
          if (!t) return "";
          const s = n(t, document.title);
          if ("" === document.referrer || !o) return s;
          const i = new URL(document.referrer);
          return location.hostname === i.hostname
            ? s
            : `${n(o, i.hostname)}<br>${s}`;
        })(s.time, s.message.welcome, s.message.referrer),
        7e3,
        9,
      );
  }
  ...
  await a.loadModel(""),
    ...
    null === (o = document.getElementById("waifu")) ||
      void 0 === o ||
      o.classList.add("waifu-active");
}
window.initWidget = function (e) {
  ...
  if ("true" === localStorage.getItem("waifu-disabled")) return;
      ...
      null == t || t.classList.remove("waifu-toggle-active"),
        (null == t ? void 0 : t.getAttribute("first-time"))
          ...
          : (localStorage.removeItem("waifu-display"),
            null === (o = document.getElementById("waifu")) ||
              void 0 === o ||
              o.classList.remove("waifu-hidden"),
            setTimeout(() => {
              var e;
